...
#define clockPin   3
#define latchPin   4
#define oePin  -1  // set to -1 to not use the enable pin (its optional)
...
int homeHoleMinInterval_ms = 700;  //The min time the home hole should be seen
int posHoleMinInterval_ms = 75;     //The min time a position hole should be seen
...
void setup()
{
  ...
  psThresh[0] = 250;  //Motor 1. Threshold for when the light hits the photosensor
  psThresh[1] = 300;  //Motor 2
  psThresh[2] = 250;  //Motor 3
  for(int i=0;i<3;i++)
...
void SetMotorPosition(int motorIndex, int motorPosition)
{
  ...
  //debugging end
  //Turn on LED
  tlc.setPWM(motorLEDPinArray[motorIndex],4095);
  tlc.write();
  
  //Move motor
  tlc.setPWM(motorPWMPinArray[motorIndex],4095);
  tlc.write();
  ...
  while(homeFound==false)
  {
    ...
    else if(psIn<psThresh[motorIndex] && isOverHole==true)
    {
      ...
      if(currentTime-firstHoleTime>homeHoleMinInterval_ms)
      {
        homeFound=true;
      ...
      //stop motor
 tlc.setPWM(motorPWMPinArray[motorIndex],0);
 tlc.write();
 ...
 //Turn off LED
  tlc.setPWM(motorLEDPinArray[motorIndex],0);
  tlc.write();
}
